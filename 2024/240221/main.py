...
#(render_template :화면 덮어씌우는것),
## custom database module로드
import database
import pandas as pd
## database에 있는 myDB1 class todtjd
mydb = database.MyDB(
    _host = '172.16.12.155',
    _port = 3306,
    _user = 'ubion',
...
def login():
    ...
    user_password = request.args['_password']
    print(request.args)
    print('ID:',user_id)
    print('PASS',user_password)
    #데이터베이스에서 user table에서 유저가 입력한 아이디 패스워드가 존재하나?
    sql = """
        select
        *
        from
        ...
        password = %s
    """
    result = mydb.sql_query(sql, user_id, user_password)
    print(result)
    if result:
        return "로그인 성공"
    else:
...
# localhost:8080:/login2주소로 post방식으로 요청을 보냈을 때
@app.route("/login2", methods={'post'})
def login2():
    # post 방식으로 유저가 서버에게 데이터를 보낼 때 (데이터를 숨겨서 보내는 방법)
    ...
    if result:
        user_name = result[0]['name']
        # 로그인이 성공했을 때 새로운 페이지를 보여주고 해당 페이지에 로그인을
        # 한 유저의 이름과 인사문구 출력
...
def view_corona():
    ...
    return render_template('corona.html',
                           columns = cols, values= val,
                           c_cnt = colsCnt, v_cnt = valCnt,
                           axis_x = x, axis_y = y)
...
# main.py에서
# {{_name}} 님 안녕하세요 {{}}중괄호 두 개는 변수의 이름을 그대로 대입할 때
# {} 중괄호 한 개는 파이썬 코드를 넣어준다. {%python code%}   
