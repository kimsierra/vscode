...
AAPL = pd.read_csv(r"C:\Users\SAMSUNG\Documents\vscode\2024\invest\data\AAPL.csv", index_col='Date')
AMZN = pd.read_csv(r"C:\Users\SAMSUNG\Documents\vscode\2024\invest\data\AMZN.csv", index_col='Date')
BND = pd.read_csv(r"C:\Users\SAMSUNG\Documents\vscode\2024\invest\data\BND.csv", index_col='Date')
...
class Invest:
    # 생성자 함수 : class를 생성할 때 최초로 한번 실행이 되는 함수
    def __init__(
           self,
           _df,
           _col = 'Adj Close',
        ...
        # Date 가 컬럼에 포함되어 있다면 인덱스로 변경
        if 'Date' in _df.columns:
            _df.set_index('Date', inplace=True)
        # 인덱스를 시계열 데이터로 변경
        _df.index = pd.to_datetime(_df.index, format='%Y-%m-%d')

        # _df의 결측치와 무한대 제거하고 기준이 되는 컬럼만 두고 나머지 
